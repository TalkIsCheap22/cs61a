...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    errors=[]
    def withdraw(amount,inputPassword):
        nonlocal errors
        if inputPassword==password and len(errors)<3:
            nonlocal balance
            if amount>balance:
                return 'Insufficient funds'
            balance-=amount
...
def merge(incr_a, incr_b):
    ...
    while next_a!=None or next_b!=None:
        if next_a==next_b:
            yield next_a
            next_a, next_b = next(iter_a, None), next(iter_b, None)
        elif next_a==None or (next_a!=None and next_b!=None and next_a>next_b):
            yield next_b
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    callRes=withdraw(0,old_pass)
    if type(callRes)==str:
        return callRes
    def takeMoney(amount,password):
...
def remainders_generator(m):
    ...
    def remainder_n_generator(n):
        current=m if n==0 else n
        while(True):
            yield current
            current+=m
    current_n=0
    while(current_n<m):
        yield remainder_n_generator(current_n)
        current_n+=1
