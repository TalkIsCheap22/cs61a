...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        "*** YOUR CODE HERE ***"
        def composed_func(f,g):
            def cal(x):
                return f(g(x))
            return cal
        return composer(composed_func(func,g))
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper_limit_binary(n,m):#m是分组中最小的二进制位数（规模不低于2的m次方）
        if n<1 or 2**m>n:
            return 0
        elif n==2**m:
            return 1
        return helper_limit_binary(n,m+1)+helper_limit_binary(n-2**m,m)
    return helper_limit_binary(total,0)
...
def move_stack(n, start, end):
    ...
    if n==1:
        ...
        print(f"Move the top disk from rod {start} to rod {middle}")
        print(f"Move the top disk from rod {start} to rod {end}")
        print(f"Move the top disk from rod {middle} to rod {end}")
    else:
