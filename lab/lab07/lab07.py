def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item]+node for node in nested_list]
...
def subseqs(s):
    ...
    if not s:
        ...
        nextLayer=subseqs(s[1:])
        return nextLayer+insert_into_all(s[0],nextLayer)
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(s[0], subseq_helper(s[1:],s[0])) +subseq_helper(s[1:],0)
...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    your,full=0,0
    def assignment(key,grade):
        nonlocal your,full
        fullScore=class_assignments[key]
        your+=grade
        full+=fullScore
        return your/full
    return assignment
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n<3:
        return 1
    return sum([num_trees(i)+num_trees(n-i) for i in range(1,n,1)])
...
def make_advanced_counter_maker():
    ...
    def make_advanced_counter():
        ...
        def advanced_counter(command):
            nonlocal localNumber,globalNumber
            "*** YOUR CODE HERE ***"
            if command=='count':
                localNumber+=1
