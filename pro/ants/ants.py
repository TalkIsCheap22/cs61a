...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current,cnt=self.place,self.min_range
        ...
        flag=False
        while not flag:
            if cnt>self.max_range or current==beehive:
                break
            if current.bees!=[]:
                flag=True
            else:
                cnt+=1
                current=current.entrance
        return rANTdom_else_none(current.bees) if flag else None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        harm=amount+self.damage if self.place is None else amount
        for bee in bees_copy:
            Insect.reduce_armor(bee,harm)
        # END Problem 5
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.exist_real:
            self.real=True
            QueenAnt.exist_real=True
        else:
            self.real=False
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        while current is not None:
            if current.ant is not None:
                if not current.ant.doubled:
                    current.ant.damage*=2
                    current.ant.doubled=True
                if isinstance(current.ant,ContainerAnt) and current.ant.contained_ant is not None:
                    if not current.ant.contained_ant.doubled:
                        current.ant.contained_ant.damage*=2
                        current.ant.contained_ant.doubled=True
